...
def test(args, model, device, test_loader, fea_loader, tar_loader, centers,m0,m1,m2,m3,v0,v1,v2,v3):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(test_loader):
            ...
            for i in range(0, 64):
                if distance_centers[i, 0] ** 0.5 > (m0 + 3 * v0):
                    if distance_centers[i, 1] ** 0.5 > (m1 + 3 * v1):
                        if distance_centers[i, 2] ** 0.5 > (m2 + 3 * v2):
                            if distance_centers[i, 3] ** 0.5 > (m3 + 3 * v3):
...
def main():
    ...
    all_close_test_2=r'C:\Users\alpka\OneDrive\Masaüstü\train_door_pressure_classification-masterr\source_code\all_close_test_2.npy'
    data_test = np.load(all_close_test_2)
    #print(data_test.shape)
    saved_model_close=r'C:\Users\alpka\OneDrive\Masaüstü\train_door_pressure_classification-masterr\source_code\saved_model_close.pt'
    model = torch.load(saved_model_close).to(device)
    centers_close=r'C:\Users\alpka\OneDrive\Masaüstü\train_door_pressure_classification-masterr\source_code\centers_close.txt'
    centers = torch.load(centers_close).to(device)
    ...
    m0, m1, m2, m3, v0, v1, v2, v3 = dist(args, model.deatch().cpu(), device, data_train, fea_loader, tar_loader, centers.deatch().cpu())
