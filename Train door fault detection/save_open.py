...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    loss_weight = 0.5
    centerloss = CenterLoss(4, 2).to(device)
...
def main():
    ...
    data_train=r'C:\Users\alpka\OneDrive\Masaüstü\train_door_pressure_classification-masterr\source_code\open_train.npy'
    data_train = np.load(data_train)
    # data_test = np.load('unseen_open_test_2.npy')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # if loss < 0.01:
        #     break
    torch.save(model, 'saved_model_open.pt')
    torch.save(centers, 'centers_open.txt')
